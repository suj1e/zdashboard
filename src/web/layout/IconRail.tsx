        ...
        <Tooltip key={p.mode}>
          <TooltipTrigger asChild>
            <button
              ...
              className={`relative w-9 h-9 flex items-center justify-center rounded-[var(--radius-lg)] text-base transition-all ${active === p.mode ? 'bg-primary/10 text-primary' : 'hover:bg-muted text-muted-foreground hover:scale-105'}`}
            >
              <span>{p.icon}</span>
            </button>
          </TooltipTrigger>
