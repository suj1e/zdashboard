          ...
          return (
            <DropdownMenuItem key={s.id} onClick={() => setStyle(s.id)} className="flex items-center gap-2.5 cursor-pointer">
              <span className="flex gap-0.5" aria-hidden="true">
                {s.swatch.map((c, i) => (
                  <span key={i} className="h-3 w-3 rounded-[var(--radius-sm)] inline-block border border-border/50" style={{ backgroundColor: c }} />
                ))}
              </span>
              <span className="flex-1 text-sm">{s.label}</span>
              {active && <span className="h-3.5 w-3.5 text-foreground">{icon('check', 'h-3.5 w-3.5')}</span>}
            </DropdownMenuItem>
          );
        })}
      </DropdownMenuContent>
