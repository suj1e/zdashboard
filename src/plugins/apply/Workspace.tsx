/**
 * apply 工作区:change 列表 + 详情(任务树/进度条),change 入 URL(?change=)。
 * 数据走 usePluginData(/__apply、/__apply/change),SSE files 频道失效重取;空态 kit EmptyState。
 */
...
import { EmptyState, PluginPage, Skeleton } from '../../web/kit/index.js';
...
export default function Workspace({ params }: PluginWorkspaceProps) {
  ...
  return (
    <PluginPage
      manifest={manifest}
      icon={themed}
      breadcrumb={['插件', manifest.mode, ...(change ? [change] : [])]}
      status={changes.length ? { label: `${changes.length} 个进行中`, tone: 'info' } : undefined}
    >
