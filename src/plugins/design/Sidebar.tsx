...
const DESIGN_CONFIG_SCHEMA = {
  folders: { type: 'string[]' as const, label: '扫描文件夹', default: [] },
};
...
export default function Sidebar() {
  ...
  const { config, save, loading, saving } = usePluginConfig('design', DESIGN_CONFIG_SCHEMA);

  useEffect(() => {
    if (loading) return;
    const folders = Array.isArray(config.folders) ? config.folders : [];
    const qs = folders.length > 0 ? `?path=${encodeURIComponent(folders.join(','))}` : '';
    fetch('/__design/assets' + qs, { cache: 'no-store' }).then(r => r.json()).then(setData);
  }, [config, loading]);
      ...
      <div className="border-t border-border mt-2">
        ...
        {showConfig && (
          <div className="px-3 pb-3 space-y-3">
            <ConfigField key_="folders" field={DESIGN_CONFIG_SCHEMA.folders} value={config.folders} onChange={handleConfigChange} />
