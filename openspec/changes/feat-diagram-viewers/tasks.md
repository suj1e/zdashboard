...
- [ ] 1. 共享 DiagramViewer:`src/web/viewers/DiagramViewer.tsx`(excalidraw lazy 只读画布 + drawio iframe `#R` + 损坏/超大/离线错误降级)+ 新依赖 `@excalidraw/excalidraw`(exact)
...
- [ ] 2. view 接入:`viewerFor` 增 `.excalidraw`/`.drawio` → DiagramViewer
  - 验收:路由单测两扩展命中 DiagramViewer
- [ ] 3. design 接入:categorize 增两扩展 → 新 AssetType `'diagram'`;viewers/ASSET_VIEWER_TYPES/VIEWERS 接 DiagramViewer;Sidebar 增「图表」分组;契约表测试同步
