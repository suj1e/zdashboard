...
1. **单元(vitest)**:
   - 主题格式守卫:解析 src/web/themes/*.css,断言所有「被 hsl() 消费」的颜色变量值匹配 `^\d{1,3} \d{1,3}(\.\d+)?% \d{1,3}(\.\d+)?%$`(实现期修订:terminal 系实测亦为 hsl() 消费——tailwind.config.ts:11 `hsl(var(--terminal-bg))`,实施期一并转 HSL,守卫不再豁免;shadow/radius/text 等非 HSL 变量仍排除);pixel.css 若在修复范围同样纳入。此测试防回归(未来再写入 RGB 三元组立即红)。
...
3. **边界**:slate.css 中本就正确的 HSL 变量(--success 等)不得被改动(格式守卫天然覆盖);terminal 系变量随实施期修订一并转 HSL(初版误记为 rgb 消费)。
