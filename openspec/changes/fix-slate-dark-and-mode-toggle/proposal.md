...
1. **slate 主题色彩变量格式错误**:`src/web/themes/slate.css` 的 --background/--foreground/--card/--popover/--primary/--secondary/--muted/--accent/--destructive-foreground 之外的主色全部使用 RGB 三元组(如 `--background: 15 23 42`),而全局消费端统一是 `hsl(var(--x))`(globals.css:109;terminal 系经 tailwind.config `hsl(var(--terminal-bg/fg))` 消费,同受影响)。结果:slate 深色渲染成棕橙色(HSL 语义下 15 23 42 被解析为 h=15 s=23% l=42%);浅色因 hsl 饱和度/亮度 clamp 碰巧接近白色而未被察觉,但前景/边框等同样失真。
...
1. slate.css 中所有被 `hsl(var(--x))` 消费的颜色变量一律为 HSL 三元组格式(`H S% L%`);RGB 格式清零(实施期修订:terminal 系经核实同为 hsl() 消费,已一并转 HSL,详见 design.md 修订记录)。
