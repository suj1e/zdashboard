...
- [x] 1. 删除 apply 插件:`git rm -r src/plugins/apply`;`src/cli.ts` BUILTIN_PLUGINS 移除注册项;typecheck 暴露的 host 残留引用逐一清理(App/router/usePluginData/plugins/icons-modes/manifests 等测试的 apply 用例删除或 stub 化,router merge 语义覆盖转由 stub 承接)
